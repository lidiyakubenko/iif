...
const checkButton = async ({page, link, item}) => {
...
(async () => {
    ...
    while (!result) {
        ...
        result = await checkButton({page, link: links.ps5Digital, item: 'PS5 digital edition'})

        console.log('Заходим на пс5 версию с дисководом')
        await page.goto(links.ps5)
        await page.screenshot({path: 'ps5.png', fullPage: true});

        result = await checkButton({page, link: links.ps5, item: 'PS5 с дисководом'})
